...
>
> **19-26 走更粗的时钟。** 这 8 个键是本 monitor 里唯一需要完整特征分解的部分，在 4B-A500M 形状上实测
> 188 ms/层，约占整套内科监控开销的 58%（router 与范数指标合计约 1 ms/层）。权重谱以数千步为尺度变化，
> 所以由 `gate_spectrum_interval`（默认 **`5`**）控制：每 5 个监控点读一次，第 0 个采样点即读，
> 其余指标仍每个监控点都读。设为 `1` 恢复旧行为。17 层、5 个监控点为一窗实测 14117 ms → 3152 ms（**4.48×**）。
> 跨层合批不是答案：一次 `[594, k, k]` 求解只比 18 次 `[33, k, k]` 快 1.22×，瓶颈是 594 次 `syevj` 分解本身。
