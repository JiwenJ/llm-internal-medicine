...
>
> **19-21 / 25 与 22-24 / 26 走不同的时钟，因为它们需要的东西不同。**
> `stable_rank = ‖A‖_F²/σ₁²` 只需要 σ₁，用幂迭代（100 步）求，**每个监控点都读**；
> `singular_entropy` 权衡整谱，绕不开特征分解，由 `gate_spectrum_interval`（默认 **`5`**）控制，
> 每 5 个监控点读一次（第 0 个采样点即读）。设为 `1` 恢复每点都读。
>
> 17 层、5 个监控点为一窗，同进程实测：原实现 1071 ms/监控点 → 现在 304 ms/监控点（**3.52×**）。
> 其中 srank 约 90 ms/点、entropy 约 1070 ms 但只在 1/5 的点上付。`eigvalsh` 占原路径的 **99.7%**
> （Gram 那次 matmul 只有 8.3 ms），所以跨层合批不是答案——一次 `[594,k,k]` 只比 18 次 `[33,k,k]` 快 1.22×。
>
> **口径提示**：幂迭代的误差是单侧的（σ₁ 被低估 → srank 偏高）。在随机 `[32,512,512]`（谱最平、
> 收敛最慢的最坏情况）上实测：30 步平均偏高 1.6%、最差专家 4.7%；**100 步平均 0.2%、最差 1.2%**；
> 200 步最差 0.8%。真实训练过的 gate 谱是衰减的，所以这是上界。秩塌缩——也就是这个指标要抓的情况——
> 恰好是幂迭代精确的那一侧。跨越本次改动比较 srank 曲线时请记住这 ≤1.2% 的系统性抬升。
